...
mqtt.on('connect', () => {
    ...
    mqtt.publish(`${config_1.config.name}/connected`, ohConnected ? '2' : '1', { qos: 0, retain: true });
});
mqtt.on('close', () => {
    if (mqttConnected) {
        mqttConnected = false;
...
oh.on('sign', (signs) => {
    ...
    signs.forEach((sign) => {
        mqtt.publish(`${config_1.config.name}/status/${sign.name}`, sign.state.toString(), { qos: 2, retain: true });
    });
});
